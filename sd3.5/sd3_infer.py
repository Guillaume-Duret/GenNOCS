...
def main(
    ...
    **kwargs,
    ...
    if isinstance(prompt, str):
        if os.path.splitext(prompt)[-1] == ".txt":
            with open(prompt, "r") as f:
                ...
                # Select 10 random prompts (or fewer if there aren't enough)
    ...
    # Extract the object name (e.g., "mug") from the controlnet_cond_image path
    object_name = os.path.basename(os.path.dirname(controlnet_cond_image)).split("_")[0]  # Gets "mug" from ".../mug_final_depth/..."

    # Extract the base image name (e.g., "mug_prompts_0_1_depth")
    image_base = os.path.splitext(os.path.basename(controlnet_cond_image))[0]

    # Construct the new output directory
    out_dir = os.path.join(
        out_dir,  # Original base dir: "outputs/sd3.5_large_..."
        f"{object_name}_images",  # e.g., "mug_images"
        image_base + datetime.datetime.now().strftime("_%Y-%m-%dT%H-%M-%S"),  # e.g., "mug_prompts_0_1_depth"
    )
    
    os.makedirs(out_dir, exist_ok=False)
    print("controlnet_cond_image : ", controlnet_cond_image)
    print("out_dir : ", out_dir)

    # Save used prompts to the output directory after it's created
    if isinstance(prompt, str) and os.path.splitext(prompt)[-1] == ".txt":
        prompt_save_path = os.path.join(out_dir, "used_prompts.txt")
