...
mkdir -p /lustre/fsn1/projects/rech/tya/ubn15wo/Final_3D_GEN
mkdir -p /lustre/fsn1/projects/rech/tya/ubn15wo/Final_3D_GEN/Fruits_data/
mkdir -p /lustre/fsn1/projects/rech/tya/ubn15wo/Final_3D_GEN/Vegetables_data/

# Loop over fruits and submit sbatch commands
for fruit in "${fruits[@]}"; do
    # Submit the first job and capture its job ID
    job_id1=$(sbatch --parsable process_ZITS_GPU_generated_meshes2.slurm "/lustre/fsn1/projects/rech/tya/ubn15wo/Final_3D_GEN/Fruits_data/" "${fruit}")
    
    # Submit the second job with a dependency on the first job
    #sbatch --dependency=afterok:$job_id1 process_CPU_texture.slurm "/lustre/fsn1/projects/rech/tya/ubn15wo/Final_3D_GEN/Fruits_data/" "${fruit}"
done

# Loop over vegetables and submit sbatch commands
for vegetable in "${vegetables[@]}"; do
    # Submit the first job and capture its job ID
    job_id1=$(sbatch --parsable process_ZITS_GPU_generated_meshes2.slurm "/lustre/fsn1/projects/rech/tya/ubn15wo/Final_3D_GEN/Vegetables_data/" "${vegetable}")
    
    # Submit the second job with a dependency on the first job
    #sbatch --dependency=afterok:$job_id1 process_CPU_texture.slurm "/lustre/fsn1/projects/rech/tya/ubn15wo/Final_3D_GEN/Vegetables_data/" "${vegetable}"
done
