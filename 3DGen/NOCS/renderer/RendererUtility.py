...
def enable_depth_output(activate_antialiasing: bool, output_dir: Optional[str] = None, file_prefix: str = "depth_",
    ...
    # Build output node
    output_file = nodes.new(type="CompositorNodeOutputFile")
    output_file.base_path = output_dir
    output_file.format.file_format = "PNG"
    # output_file.format.color_mode = 'BW'
    # print("hello color depth")
    # print(output_file.format.color_depth)
    output_file.format.color_depth = '16'
    # print("hello color depth")
    # print(output_file.format.color_depth)
    output_file.file_slots.values()[0].path = file_prefix
    # output_file.file_slots[0].path = file_prefix

    # Link the nodes
    links.new(render_layer_node.outputs["Depth"], normalize_node.inputs['Value'])
    links.new(normalize_node.outputs['Value'], output_file.inputs['Image'])
