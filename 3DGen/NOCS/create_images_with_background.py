...
def save_mask(img, index: int, out_dir: str):
    ...
    array -= 2  # Minus one for the background, minus one for the ground_obj
...
# from .save_images import folderToCategory
...
def create_scene(mesh_path: str, ground_obj: bproc.types.MeshObject, json_file_path: str) -> (
    ...
    def sample_pose(obj: bproc.types.MeshObject):
        obj.set_rotation_euler(bproc.sampler.uniformSO3(False, False, False))
    ...
    # # Set the rotation of the object to the identity matrix
    #     obj.set_rotation_mat(identity_matrix)
...
if __name__ == "__main__":
    ...
    for _ in range(int(args.shots_number)):
        ...
        # bproc.camera.set_intrinsics_from_blender_params(lens_unit="MILLIMETERS",lens=100)
        # bproc.camera.set_intrinsics_from_blender_params(clip_end=20)
    ...
    bproc.renderer.set_output_format(enable_transparency=True,color_depth=16)
    bproc.renderer.enable_depth_output(activate_antialiasing=False,output_dir="/home/ybourennane/blenderProc/shapo-main/blenderproc/utils/")


    # render the whole pipeline
    data = bproc.renderer.render()
    data.update(bproc.renderer.render_nocs())
    data.update(bproc.renderer.render_segmap(map_by="instance", render_colorspace_size_per_dimension=len(objs) + 2))
