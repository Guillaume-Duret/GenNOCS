...
def create(room_name):  
    ...
    model_objects = "/home/mazurrrak/ECL/Semester2/PO/Omni6D/data/object/raw_scans/"
    ...
    for cate in sorted(cates):
        ...
        for obj in objects:
            if os.path.exists(os.path.join(model_objects, split, cate, obj, 'Scan/Scan.obj')):
                models.append(os.path.join(model_objects, split, cate, obj, 'Scan/Scan.obj'))
    print(len(models))
    ...
    for obj_id in range(len(objs_path)):
        ...
        scale_path = objs_path[obj_id].replace('Scan.obj', 'scale.json')
